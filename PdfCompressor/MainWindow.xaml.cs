...
namespace PdfCompressor
{
    public partial class MainWindow : Window
    {
        ...
        // Flags to ensure admin warning shows only once per language
        private static bool warnedUk = false;
        private static bool warnedEn = false;
        ...
        // Event fired when main window is fully loaded
        private void MainWindow_Loaded(object sender, RoutedEventArgs e)
        {
            if (!IsRunAsAdmin()) return;

            string lang = Thread.CurrentThread.CurrentUICulture.TwoLetterISOLanguageName;
            bool shouldShow = (lang == "uk" && !warnedUk) || (lang == "en" && !warnedEn);

            if (shouldShow)
            {
                // Prevent repeated warnings
                if (lang == "uk") warnedUk = true;
                if (lang == "en") warnedEn = true;

                // Delay the dialog slightly
                var timer = new System.Windows.Threading.DispatcherTimer { Interval = TimeSpan.FromSeconds(1) };
                timer.Tick += (s, args) =>
                {
                    timer.Stop();
                    string msg = LocalizationManager.GetString("RunAsAdminWarningMessage");
                    string caption = LocalizationManager.GetString("Info");

                    Log(LocalizationManager.GetString("RunAsAdminWarningLog"));
        ...
        private void About_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    FileName = "https://the-inet.vercel.app/PdfCompressor",
                    UseShellExecute = true
                });
            }
            catch (Exception ex)
        ...
        private async Task RunWithAutoLoading(Func<Task> operation, int delayBeforeShowing = 300)
        {
            ...
            finally
            {
                ...
                if (loadingShown)
                {
                    Dispatcher.Invoke(() =>
                    {
                        LoadingText.Text = LocalizationManager.GetString("Ready") ?? "Готово!";
                    });
